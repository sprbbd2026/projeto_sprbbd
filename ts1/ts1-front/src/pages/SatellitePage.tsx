            ...
            <div className="mx-auto w-full max-w-6xl space-y-8">
                ...
                <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div className="rounded-2xl p-6" style={{ background: "var(--surface)", boxShadow: "var(--shadow)" }}>
                        <p className="text-sm" style={{ color: "var(--text-h)" }}>Total de Satélites</p>
                        <h2 className="mt-2 text-3xl font-bold" style={{ color: "var(--white)" }}>
                            {satellites.length}
                        </h2>
                    </div>

                    <div className="rounded-2xl p-6" style={{ background: "var(--surface)", boxShadow: "var(--shadow)" }}>
                        <p className="text-sm" style={{ color: "var(--text-h)" }}>Satélites em operação</p>
                        <h2 className="mt-2 text-3xl font-bold">
                            {satellites.filter((s) => s.sat_status.toLowerCase() === "operacional").length}
                        </h2>
                    </div>

                    <div className="rounded-2xl p-6" style={{ background: "var(--surface)", boxShadow: "var(--shadow)" }}>
                        <p className="text-sm" style={{ color: "var(--text-h)" }}>Constelações diferentes</p>
                        <h2 className="mt-2 text-3xl font-bold">
                            {new Set(satellites.map((s) => s.con_id).filter((id) => id !== null)).size}
                        </h2>
                    </div>
                </section>
                ...
                <section className="rounded-2xl p-6" style={{ background: "var(--surface)", boxShadow: "var(--shadow)" }}>
                    ...
                    {loading ? (
                        <div className="py-10 text-center text-gray-400">Carregando satélites...</div>
                    ) : satellites.length === 0 ? (
                        <div className="py-10 text-center text-gray-400">Nenhum satélite cadastrado.</div>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="border-b" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
                                        <th className="py-3 text-left text-sm text-gray-400">SAT ID</th>
                                        <th className="py-3 text-left text-sm text-gray-400">PRN</th>
                                        <th className="py-3 text-left text-sm text-gray-400">SVN</th>
                                        <th className="py-3 text-left text-sm text-gray-400">Constelação</th>
                                        <th className="py-3 text-left text-sm text-gray-400">Offset Relógio</th>
                                ...
                                <tbody>
                                    {satellites.map((satellite) => (
                                        <tr key={satellite.sat_id} className="border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                                            <td className="py-4" style={{ color: "var(--text)" }}>
                                                {satellite.sat_id}
                                            </td>
                                            <td className="py-4" style={{ color: "var(--text-h)" }}>
                                                {satellite.sat_codigo_prn ?? "—"}
                                            </td>
                                            <td className="py-4" style={{ color: "var(--text-h)" }}>
                                            ...
                                            <td className="py-4">
                                                <span className={`rounded-full px-3 py-1 text-xs font-medium ${statusStyles[satellite.sat_status] || "bg-gray-500/20 text-gray-300"}`}>
                                            ...
                                            <td className="py-4">
                                                <div className="flex gap-3 items-center">
                                                    <button
                                                        onClick={() => navigate(`/satellites/edit/${satellite.sat_id}`)}
            ...
            {confirmDeleteId !== null && (
                <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
                    <div className="bg-[var(--surface)] p-6 rounded-xl shadow-lg w-full max-w-sm text-center space-y-4">
                        ...
                        <p className="text-gray-300">Tem certeza que deseja excluir este satélite?</p>
                        <div className="flex justify-center gap-4 mt-4">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    setSatellites((prev) => prev.filter((s) => s.sat_id !== confirmDeleteId));
